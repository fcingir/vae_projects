...
Example Usage: 
	python draw_GRAY_automated_fashion_validation.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_GRAY_automated_fashion_validation.py --data_dir=/tmp/draw --read_attn=False --write_attn=False
...
def DRAW(test_data, dim_representation, nodes, restore_model_filepath, save_path):
    ...
    saver.restore(sess, restore_model_filepath) # to restore from model (.ckpt file), uncomment this line

    train_data = test_data
    del test_data # memory, needs refactoring
    ...
    for e in range(epochs):
            ...
            saver.restore(sess, restore_model_filepath) # to restore from model, uncomment this line
            print('Epoch' + str(e+1))
    ...
    out_file=os.path.join(save_path,"\\draw_data_fashion_ndim_"+str(dim_representation)+"_"+str(nodes)+".npy")
    np.save(out_file,[canvases,Lxs,Lzs,reconstructed_images])
...
dataset_prep = np.asarray([prepare_dataset_fashion_mnist])
...
def train_all_datasets(add_list_of_datasets):
    ...
    for i in range(len(dataset_prep)):
        #filename = 'dataset_'+str(i)+'_DRAW'
        filename = 'dataset_1_DRAW'
    ...
    for i in range(len(dataset_prep)):
        ...
        model_hyperparameters = [#[2,256],[10,256],[20,256],[100,256],
                         ...
                         [2,X_test.shape[1]],[10,X_test.shape[1]],[20,X_test.shape[1]],[100,X_test.shape[1]]]
        ...
        for j in model_hyperparameters:
            ...
            weights_name = os.path.join(os.getcwd(),"drawmodel_fashion_ndim"+str(j[0])+"_"+str(j[1])+".ckpt") #variable name might be confusing, change this
            DRAW(X_test, dim_representation = j[0], nodes = j[1], restore_model_filepath = weights_name, save_path = path_dataset) #(train_set, test_set, modelname, desired epochs, filename to save to)
