...
Example Usage: 
	python draw_svhn.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_svhn.py --data_dir=/tmp/draw --read_attn=False --write_attn=False

Author for Thesis Purposes: Fatih Cingir

Original implementation: Eric Jang
...
A,B = 32,32 # image width,height, depth
img_size = B*A # the canvas size
#img_size_rgb = B*A*C
enc_size = 512 # number of hidden units / output size in LSTM, 256 is in original code
dec_size = 512
...
train_iters=5000
epochs = 30
learning_rate=1e-3 # learning rate for optimizer, 1e-3 is original
...
x = tf.placeholder(tf.float32,shape=(None,3072)) # input (batch_size * (img_size*3)) - img_size*3 because 3 dimensional
...
def read_attn(x,x_hat,h_dec_prev):
    ...
    def filter_img(img,Fx,Fy,gamma,N):
        ...
        img=tf.reshape(img,[-1,B,A,C]) ### added C for number of colors
        img_t = tf.transpose(img, perm=[3,0,1,2]) # transpose
...
for t in range(T):
    c_prev = tf.zeros((batch_size,3072)) if t==0 else cs[t-1]
...
directory = 'C:\\Users\\31687\\Desktop\\draw_tensorflow\\draw\\svhn'
file = directory + '/train_32x32.mat'
input_data = sio.loadmat(file)['X']/255.0 # extracts all training images into shape of 32,32,3,73257
train_data = np.array([a.flatten() for a in input_data.T]) # arr is now a np.ndarray type of object of shape (73257, 3072)
...
for e in range(epochs):
        ...
        for i in range(period):
                ...
                if len(batch_xtrain) < batch_size: #to prevent feeding errors, probably unnecessary at this point
                    print(i)
                    #break
