...
Example Usage: 
	python draw_mnist_halfmissing2.py --data_dir=/tmp/mnist --read_attn=True --write_attn=True
	python draw_mnist_halfmissing2.py --data_dir=/tmp/mnist --read_attn=False --write_attn=False
# NOTE THIS SAVES TO //:C/tmp/mnist
...
input_data = read_idx(file)
# arr is now a np.ndarray type of object of shape 60000, 28, 28
train_data = np.array([a.flatten() for a in input_data])/255.0
# arr is now a np.ndarray type of object of shape 60000, 784

directory = 'C:\\Users\\31687\\Desktop\\draw_tensorflow\\draw\\mnist'
file = directory + '/train-labels-idx1-ubyte.gz'
y_train = read_idx(file)
# arr is now a np.ndarray type of object of shape 60000

 

## REMOVE HALF LABELS OF DATA ##

# Create incomplete dataset to train on. Removes a single label for during training phase
def label_remove(x_dataset, y_dataset, desired_label):
    indexer = []
    for i in range(0, len(y_dataset)):
        for labelnumber in desired_label:
            if y_dataset[i] == labelnumber:
                indexer.append(i)
    
    removed_x_train = np.delete(x_dataset, indexer, axis=0)
    #removed_x_test = np.delete(x_test, indexer, axis=0)
    
    return removed_x_train


undesired_labels1 = [5,6,7,8,9]
train_data = label_remove(train_data, y_train, undesired_labels1)
...
#saver.restore(sess, "drawmodel_mnist_ndim128.ckpt") # to restore from model, uncomment this line
...
out_file=os.path.join(FLAGS.data_dir,"draw_data_mnist_ndim10_secondlabelhalfmissing.npy")
np.save(out_file,[canvases,Lxs,Lzs])
print("Outputs saved in file: %s" % out_file)

ckpt_file=os.path.join(FLAGS.data_dir,"drawmodel_mnist_ndim10_secondlabelhalfmissing.ckpt")
