...
Example Usage: 
	python draw_GRAY_automated.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_GRAY_automated.py --data_dir=/tmp/draw --read_attn=False --write_attn=False
...
def DRAW(train_data, dim_representation, nodes):
    ...
    ckpt_file=os.path.join(os.getcwd(),"drawmodel_mnist_ndim"+str(dim_representation)+"_"+str(nodes)+".ckpt") # change name according to dataset here
...
dataset_prep = np.asarray([prepare_dataset_mnist]) #1 model takes about 85 minutes to train if you want 100 epochs. So cycle through them one by one. 
