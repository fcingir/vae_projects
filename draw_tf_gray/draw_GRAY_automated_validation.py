...
Example Usage: 
	python draw_GRAY_automated_validation.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_GRAY_automated_validation.py --data_dir=/tmp/draw --read_attn=False --write_attn=False
...
def DRAW(test_data, dim_representation, nodes, restore_model_filepath, save_path):
    ...
    out_file=os.path.join(save_path,"\\draw_data_mnist_ndim_"+str(dim_representation)+"_"+str(nodes)+".npy")
...
def train_all_datasets(add_list_of_datasets):
    ...
    for i in range(len(dataset_prep)):
        ...
        for j in model_hyperparameters:
            ...
            weights_name = os.path.join(os.getcwd(),"drawmodel_mnist_ndim"+str(j[0])+"_"+str(j[1])+".ckpt") #variable name might be confusing, change this
