...
Example Usage: 
	python draw_fashion_validation.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_fashion_validation.py --data_dir=/tmp/draw --read_attn=False --write_attn=False
...
tf.compat.v1.disable_eager_execution() # enable learning for training set
###
...
batch_size=1 # training minibatch size, 64 is in original code
train_iters=10000
epochs = 1
...
with gzip.open(filePath_train_label, 'rb') as trainLbpath:
     y_test = np.frombuffer(trainLbpath.read(), dtype=np.uint8,  ################# note: the variable names are very messy!
                               offset=8)

with gzip.open(filePath_train_set, 'rb') as trainSetpath:
     test_data = np.frombuffer(trainSetpath.read(), dtype=np.uint8,
                               offset=16).reshape(len(y_test), 784)/255.0
...
fetches=[]
fetches.extend([Lx,Lz,train_op,reconstruction_error, cs[-1]])
Lxs=[0]*train_iters
Lzs=[0]*train_iters
reconstruction_errors =[0]*train_iters ##### Test
reconstructed_images = [0]*train_iters
...
saver.restore(sess, "/tmp/draw/drawmodel_mnist_ndim10_256.ckpt") # to restore from model, uncomment this line


train_datasize = test_data.shape[0]

period = int(train_datasize/batch_size) #rounds down
for e in range(epochs):
        saver.restore(sess, "/tmp/draw/drawmodel_mnist_ndim10_256.ckpt") # HIGHLY EXPERIMENTAL !!!!!!!!!!!!! to restore from model, uncomment this line 
        print('Epoch' + str(e+1))

        for i in range(period):
                saver.restore(sess, "/tmp/draw/drawmodel_mnist_ndim10_256.ckpt") # to restore from model, uncomment this line
                batch_xtrain = test_data[i * batch_size:(i+1) * batch_size] # Fatty experiment

                feed_dict={x:batch_xtrain}
                results=sess.run(fetches,feed_dict)
                Lxs[i],Lzs[i],_, reconstruction_errors[i], reconstructed_images[i] =results
                if i%100==0:
                        print("iter=%d : VALIDATION Lx: %f Lz: %f Recon_error: %f" % (i,Lxs[i],Lzs[i], reconstruction_errors[i]))

## TRAINING FINISHED ##

## RECONSTRUCTION PROBABILITY ##
                        
#Introduce anomalous dataset:


                        
# Create incomplete dataset to train on. Removes a single label for during training phase
def label_remove(desired_label):
        indexer = []
        for i in range(0, len(y_train)):
                if y_train[i] == int(desired_label):
                        indexer.append(i)
    
        removed_x_train = np.delete(train_data, indexer, axis=0)
        #removed_x_test = np.delete(x_test, indexer, axis=0)
    
        return removed_x_train#, removed_x_test

#x_train_3 = label_remove(3)[:batch_size,:]




## END RECONSTRUCTION PROBABILITY ##


canvases=sess.run(cs,feed_dict) # generate some examples
canvases=np.array(canvases) # T x batch x img_size

out_file=os.path.join(FLAGS.data_dir,"EXPERIMENTAL_draw_data_fashion_ndim10_256_validation.npy")
np.save(out_file,[canvases,Lxs,Lzs,reconstruction_errors, reconstructed_images])
print("Outputs saved in file: %s" % out_file)

#ckpt_file=os.path.join(FLAGS.data_dir,"drawmodel_fashion_ndim10_256.ckpt")
