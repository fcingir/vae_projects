...
Example Usage: 
	python draw_mnist_validation.py --data_dir=/tmp/draw --read_attn=True --write_attn=True
	python draw_mnist_validation.py --data_dir=/tmp/draw --read_attn=False --write_attn=False
...
train_iters=10000
epochs = 1 #1 epoch, pass through all data for training
...
#
#_,input_data, _, _ = prepare_dataset_mnist()
#
## arr is now a np.ndarray type of object of shape 10000, 28, 28

import gzip
import numpy as np

directory = 'C:\\Users\\31687\\Desktop\\draw_tensorflow\\draw\\mnist'
...
with gzip.open(filePath_train_set, 'rb') as trainSetpath:
     train_data = np.frombuffer(trainSetpath.read(), dtype=np.uint8,
                               offset=16).reshape(len(y_train), 784)/255.0

del y_train # save memory

train_data = train_data[:100]
# arr is now a np.ndarray type of object of shape 10000, 784



fetches=[]
fetches.extend([Lx,Lz,train_op,reconstruction_error, cs[-1]) # outputs in 28x28 format instead of 1,784
Lxs=[0]*train_iters
Lzs=[0]*train_iters
...
for e in range(epochs):
        ...
        #tf.compat.v1.disable_eager_execution() # Disable learning for validation set 

        period_test = int(test_data.shape[0]/batch_size) #rounds down
        for i in range(period_test):
                saver.restore(sess, "/tmp/draw/drawmodel_mnist_ndim10_256.ckpt")
                batch_xtest = test_data[i * batch_size:(i+1) * batch_size] # Fatty experiment

                feed_dict={x:batch_xtest}
...
#out_file=os.path.join(FLAGS.data_dir,"drawmodel_mnist_ndim10_256_HALFMISSING1_validation.npy")
out_file=os.path.join(FLAGS.data_dir,"TESTYTESTY_MNIST.npy")
np.save(out_file,[canvases,Lxs,Lzs,reconstruction_errors, reconstructed_images])
print("Outputs saved in file: %s" % out_file)

#ckpt_file=os.path.join(FLAGS.data_dir,"drawmodel_mnist_ndim10_256.ckpt")
